  ...
  const unmatchedNodes = nodeElements.filter((d: Node) => {
    ...
    if (nodeNameOrGroup.length > 0) {
      for (const highlightedNode of highlightedNodes) {
        const highlightedNodeNameOrGroup = utils.getNodeNameOrGroup(
          highlightedNode
        );
...
export function GetUnmatchedNodes(
  ...
  return nodeElements.filter(
    (a) =>
      GetMatchedNodes(highlightedNodes, nodeElements).data().indexOf(a) === -1
  );
}

function DoesLinkMatch(
  highlightedNodes: Node[],
  l: Link,
  ...
  for (const highlightedNode of highlightedNodes) {
    ...
    if (onlyLinksWithHighlightedSourceAndTarget === true) {
      ...
      utils.strEquals(nodeNameOrGroup, linkSourceNameOrGroup) ||
      utils.strEquals(nodeNameOrGroup, linkTargetNameOrGroup)
    ) {
      return true;
    }
  }
...
export function GetMatchedLinks(
  ...
  const matchedLinks = linkElements.filter((l: Link) => {
    return DoesLinkMatch(
      highlightedNodes,
      l,
      onlyLinksWithHighlightedSourceAndTarget
    );
  });
  return matchedLinks;
}
...
export function GetUnmatchedHulls(
  ...
  return hullElements.filter(
    (a) =>
      GetMatchedHulls(highlightedHulls, hullElements).data().indexOf(a) === -1
  );
}
...
export function GetNeighbourNodes(
  ...
  const highlightedNodeNamesOrGroups: string[] = highlightedNodes.map((n) =>
    utils.getNodeNameOrGroup(n)
  );
  for (const highlightedLink of highlightedLinks) {
    const linkSourceNameOrGroup = utils.getLinkSourceNameOrGroup(
      highlightedLink
    );
    const linkTargetNameOrGroup = utils.getLinkTargetNameOrGroup(
      highlightedLink
    );
    if (
      highlightedNodeNamesOrGroups.some((nog) =>
        utils.strEquals(nog, linkSourceNameOrGroup)
      ) &&
      !highlightedNodeNamesOrGroups.some((nog) =>
        utils.strEquals(nog, linkTargetNameOrGroup)
      )
    ) {
