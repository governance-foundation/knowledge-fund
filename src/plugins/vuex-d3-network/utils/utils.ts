  ...
  for (const node of nodes) {
    if (node.group) {
      ...
      groupedDictionaryOfNodes[group] =
        !groupedDictionaryOfNodes[group] ||
        (groupedDictionaryOfNodes[group].size || nodeRadiusSizes.default) <
          (node.size || nodeRadiusSizes.default)
          ? node
          : groupedDictionaryOfNodes[group];
      if (!groupedDictionaryOfNodes[group].size) {
        groupedDictionaryOfNodes[group].size = getRadius(
          groupedDictionaryOfNodes[group]
        );
      }
    }
    ...
    if (groupedDic.size === nodeRadiusSizes.default) {
      ...
      for (const jNode of groupedDic.nodes!) {
        ...
        for (const kLink of links) {
          ...
          if (
            strEquals(kLink.source, nodeName) ||
            strEquals(kLink.sourceChild, nodeName)
          ) {
            for (const lNode of groupedDic.nodes!) {
              if (
                strEquals(kLink.target, lNode.name) ||
                strEquals(kLink.targetChild, lNode.name)
              ) {
                isInternalLink = true;
                break;
              }
            ...
            strEquals(kLink.target, nodeName) ||
            strEquals(kLink.targetChild, nodeName)
          ) {
            for (const lNode of groupedDic.nodes!) {
              if (
                strEquals(kLink.source, lNode.name) ||
                strEquals(kLink.sourceChild, lNode.name)
              ) {
                isInternalLink = true;
                break;
              }
        ...
        if (
          ...
          nodeWithLinkCount.numberOfInternalLinks >
            nodeWithMostNumberOfInternalLinks.numberOfInternalLinks
        ) {
...
export function getRadius(d: Node): number {
  ...
  if (d.size) {
    ...
    for (const iNode of d.nodes) {
      biggestSize =
        (iNode.size || nodeRadiusSizes.default) > biggestSize
...
export function convexHulls(nodes: Node[]): Hull[] {
  ...
  for (const n of nodes) {
    ...
    if (!Object.prototype.hasOwnProperty.call(hulls, i)) {
      hulls[i] = [];
    }
...
// Reusable transition
export function transitionLinear(
  duration: number
): d3.Transition<d3.BaseType, any, any, any> {
...
export function getLinkGradientId(d: Link) {
  const gradientId: string =
    "linkGrad-" + getLinkSourceName(d) + getLinkTargetName(d);
  return gradientId
    .replace(/ /g, "_")
    .replace(/\(|\)/g, "__")
...
export function darkenIfInvertedBackground(
  color: string,
  isInverted: boolean
): string {
